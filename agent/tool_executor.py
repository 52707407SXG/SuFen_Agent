...
if os.getenv("SUFEN_AGENT_MODE") == "1":
    def get_active_env(_task_id: str | None = None):
        return None
else:
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("SUFEN_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid SUFEN_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current SuFen process before the normal turn-end persistence path runs.
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for tool_call in tool_calls:
        ...
        # the legacy runtime lesson: hooks must observe the real tool name.
        #
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _block_msg is not None:
            ...
            def _execute(next_args: dict) -> Any:
                ...
                if not session_db:
                    from sufen_state import format_session_db_unavailable
