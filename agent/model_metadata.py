...
from sufen_constants import OPENROUTER_MODELS_URL
...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. SuFen also honours SUFEN_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("SUFEN_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_sufen_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run SuFen-Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent SuFen from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for SuFen's system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets SuFen minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from sufen_constants import get_sufen_home
    return get_sufen_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # SuFen grow conversations past the runtime limit and Ollama
...
def _resolve_nous_context_length(
    ...
    """Resolve inherited portal model context length.
...
def get_model_context_length(
    ...
    if custom_providers and base_url and model:
        try:
            from sufen_cli.config import get_custom_provider_context_length
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # inherited portal: the portal /v1/models endpoint is authoritative.
            ...
            # cost amortise to ~0 within a process.
                ...
                logger.debug(
                    "Bypassing persistent cache for %s@%s (inherited portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from sufen_cli.models import get_copilot_model_context
    ...
    # ``SuFen-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets SuFen sends to providers:
