...
from sufen_constants import get_sufen_home, get_skills_dir, is_wsl
...
_SUFEN_MD_NAMES = (".sufen.md", "SUFEN.md")


def _find_sufen_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.sufen.md`` or ``SUFEN.md``.
    ...
    # could pick up a .sufen.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SUFEN_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are SuFen, My Stand's archive strategist and analyst for owners, "
    "clients, brokers, and after-sale contexts. You are not a generic chatbot, "
    "not Miner, not customer service, and not a broad search bot. Your job is "
    "to reason from authorized archive context, knowledge graphs, broker feature "
    "cards, and scoped memory, then advise the broker on the person, the situation, "
    "and the next action."
)

SUFEN_AGENT_HELP_GUIDANCE = (
    "You run as SuFen-Agent. When the user needs help configuring, running, "
    "or troubleshooting SuFen itself, prefer the local RUNBOOK.md, SOURCE_MAP.md, "
    "DISCOVERY.md, and .env.example in this repository. Treat SuFen's My Stand "
    "authorization boundary as stricter than any inherited runtime capability."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.sufen/kanban.db`. Your task id is in "
    "`$SUFEN_KANBAN_TASK`; your workspace is `$SUFEN_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SUFEN_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SUFEN_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SUFEN_KANBAN_BRANCH:-wt/$SUFEN_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SUFEN_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`sufen profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `sufen kanban <verb>` for board operations. Use "
...
#
# The inherited runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to this Python
...
# Inspired by patterns from OpenAI's GPT-5.4 prompting guide and upstream tool-surface work.
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `sufen computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that SuFen "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the SuFen terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the SuFen WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where SuFen itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host SuFen runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where SuFen itself is running. The host OS, home, and cwd "
                f"of the SuFen process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where SuFen "
    ...
    # SuFen desktop GUI — any agent running under the desktop app should know
    # it. SUFEN_DESKTOP marks the backend powering the chat; SUFEN_DESKTOP_TERMINAL
    # marks a sufen launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("SUFEN_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("SUFEN_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the SuFen desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps SuFen
    ...
    extra = (os.getenv("SUFEN_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
            from sufen_cli.config import load_config
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sufen_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sufen/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("SUFEN_PLATFORM")
        or get_session_env("SUFEN_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot SuFen itself - its CLI, config, models, providers, tools, "
            "skills, gateway, plugins, or any feature - load the local SuFen runbook and "
            "source map first. They define the supported commands and boundaries so you do "
            "not guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact inherited managed-capability block for the system prompt."""
    try:
        from sufen_cli.nous_subscription import get_nous_subscription_features
        from tools.tool_backend_helpers import managed_nous_tools_enabled
    except Exception as exc:
        logger.debug("Failed to import inherited managed-capability helper: %s", exc)
        return ""
    ...
    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via inherited managed capability"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.nous_auth_present:
            return f"- {feature.label}: included with inherited managed capability, not currently selected"
        if feature.key == "modal" and features.nous_auth_present:
            return f"- {feature.label}: optional via inherited managed capability"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Inherited Managed Capabilities",
        "Inherited managed capabilities can include web tools, image generation, speech tools, and browser automation. Modal execution is optional.",
    ...
    lines.extend(
        [
            ...
            "If the user lacks access and asks for a capability this managed layer would unlock or simplify, suggest direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: sufen doctor, sufen doctor tools, sufen doctor terminal, sufen status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from SUFEN_HOME for inherited runtime compatibility.
    ...
    """
    try:
        from sufen_cli.config import ensure_sufen_home
        ensure_sufen_home()
    except Exception as e:
        logger.debug("Could not ensure SUFEN_HOME before loading SOUL.md: %s", e)

    soul_path = get_sufen_home() / "SOUL.md"
...
def _load_sufen_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.sufen.md / SUFEN.md — walk to git root."""
    sufen_md_path = _find_sufen_md(cwd_path)
    if not sufen_md_path:
        return ""
    try:
        content = sufen_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = sufen_md_path.name
        try:
            rel = str(sufen_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".sufen.md", context_length=context_length,
            read_path=str(sufen_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", sufen_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .sufen.md / SUFEN.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SUFEN_HOME is independent and always included when present.
    ...
    project_context = (
        _load_sufen_md(cwd_path, context_length)
    ...
    # SOUL.md from SUFEN_HOME only — skip when already loaded as identity
    if not skip_soul:
