...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SUFEN_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # SuFen runtime / deployment
    "SUFEN_HOME", "SUFEN_PROFILE", "SUFEN_GATEWAY_LOCK_DIR",
    "SUFEN_MAX_ITERATIONS", "SUFEN_MAX_TOKENS", "SUFEN_API_TIMEOUT",
    "SUFEN_REDACT_SECRETS", "SUFEN_NOUS_TIMEOUT_SECONDS",
    "_SUFEN_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SUFEN_KANBAN_DB", "SUFEN_KANBAN_WORKSPACES_ROOT", "SUFEN_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "SUFEN_KANBAN_",
    "SUFEN_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(sufen_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(sufen_home) / ".env")
