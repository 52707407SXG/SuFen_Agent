...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `sufen backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - sufen_home (str): The active SUFEN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.sufen``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "sufen").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'sufen memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], sufen_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'sufen memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``sufen_home`` is the active SUFEN_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SUFEN_HOME.

        ``sufen backup`` only walks SUFEN_HOME, so any provider state kept
        ...
        of the archive, and ``sufen import`` restores them to their original
