...
Resolution order for text tasks (auto mode):
  ...
  3. inherited portal (~/.sufen/auth.json active provider)
...
if os.getenv("SUFEN_AGENT_MODE") == "1":
...
if os.getenv("SUFEN_AGENT_MODE") == "1":
    from sufen.config import get_sufen_home as get_sufen_home
else:
    try:
        from sufen_cli.config import get_sufen_home
    except ModuleNotFoundError:
        from sufen.config import get_sufen_home as get_sufen_home
from sufen_constants import OPENROUTER_BASE_URL
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # SuFen owns auxiliary retry + provider/model fallback policy (the
    ...
    # SuFen sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let SuFen control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before SuFen triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/52707407SXG/SuFen_Agent",
    "X-Title": "SuFen-Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from sufen_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``SUFEN_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``SUFEN_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("SUFEN_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("SUFEN_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "SuFenAgent",
}
...
# inherited portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by inherited portal.
#
if os.getenv("SUFEN_AGENT_MODE") == "1":
    def _nous_portal_tags() -> dict:
        return {}
else:
...
def _nous_extra_body() -> dict:
    """Return a fresh inherited portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``sufen_cli.__version__`` is
...
# Set at resolve time — True if the auxiliary client points to inherited portal
...
_AUTH_JSON_PATH = get_sufen_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (SuFen-Agent)",
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from sufen_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("SUFEN_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``sufen_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.sufen/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from sufen_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from sufen_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from sufen_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("SUFEN_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``sufen auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``sufen_cli.auth``'s singleton runtime resolver for older
    auth-store-only logins. Returns ``None`` if the user is not authenticated
    with xAI Grok OAuth.
    """
    try:
        from sufen_cli.auth import (
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    os.getenv("SUFEN_XAI_BASE_URL", "").strip().rstrip("/")
    ...
    try:
        from sufen_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from SuFen auth store.
    ...
    try:
        from sufen_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from sufen_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from sufen_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from sufen_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from agent.managed_rate_guard import managed_rate_limit_remaining
        _remaining = managed_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping inherited portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    logger.debug("Auxiliary client: inherited portal")
    ...
    model = _NOUS_MODEL
    try:
        from sufen_cli.models import get_nous_recommended_aux_model
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the inherited portal's recommended model after a stale-model 404.
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    """
    try:
        from sufen_cli.runtime_provider import resolve_runtime_provider
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `sufen doctor` or `sufen doctor` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`sufen_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from sufen_cli.runtime_provider import _resolve_azure_foundry_runtime
        from sufen_cli.auth import AuthError
        from sufen_cli.config import load_config
    except ImportError:
        return None, None
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second sufen
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``sufen doctor aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from sufen_cli.nous_account import get_nous_portal_account_info
...
def _recoverable_pool_provider(
    ...
    if main_runtime:
        ...
        if rt_provider and rt_provider not in {"", "auto", "custom"}:
            try:
                from sufen_cli.auth import PROVIDER_REGISTRY
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "openai-codex":
            from sufen_cli.auth import resolve_codex_runtime_credentials
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into SuFen's built-in discovery
    ...
    try:
        from sufen_cli.config import load_config
        from sufen_cli.fallback_config import get_fallback_chain
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `sufen doctor` but the
    #    old OPENAI_BASE_URL lingers in ~/.sufen/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: sufen doctor to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.sufen/.env",
    ...
    if main_provider == "moa":
        try:
            from sufen_cli.config import load_config
            from sufen_cli.moa_config import resolve_moa_preset
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from sufen_cli.copilot_auth import copilot_request_headers
    ...
    # See _create_openai_client: disable SDK-internal retries so SuFen owns
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from sufen_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but inherited portal not configured (run: sufen auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: sufen doctor)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: sufen doctor -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from sufen_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: sufen doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("sufen_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from sufen_cli.models import _should_use_copilot_responses_api
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3. inherited portal (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes inherited portal product tags when the auxiliary client is backed
    by inherited portal. Returns empty dict otherwise.
    """
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from sufen_cli.providers import get_provider
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`sufen_cli.plugins.PluginContext.register_auxiliary_task`) the
    ...
    try:
        from sufen_cli.plugins import get_plugin_auxiliary_tasks
...
def call_llm(
    ...
    *,
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `sufen doctor`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (inherited portal recommendation drift) ───
