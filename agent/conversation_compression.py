    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `sufen doctor` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by SuFen "
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``sufen_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `sufen update`) to resync.",
    ...
    try:
            ...
            try:
                ...
                if in_place:
                    ...
                    try:
                        ...
                        os.environ["SUFEN_SESSION_ID"] = agent.session_id
                    # The gateway/tools session context (ContextVar + env) and the
                    # logging session context are SEPARATE mechanisms. The call above
                    # moves the former; the ``[session_id]`` tag on log lines comes
                    # from ``sufen_logging._session_context`` (set once per turn in
                    ...
                    try:
                        agent._session_db.create_session(
                            ...
                            source=agent.platform or os.environ.get("SUFEN_SESSION_SOURCE", "cli"),
                            model=agent.model,
                    ...
                    try:
                        from sufen_cli.goals import migrate_goal_to_session
        ...
        # engines (e.g. sufen-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See sufen-lcm#68. Built-in ContextCompressor
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="sufen_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
