...
upstream prefix cache warm. See ``references/system-prompt-invariant.md`` for the invariants and
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, inherited managed-capability block, tool-use
...
class _SufenRuntimeCompat:
    """Minimal runtime helpers for SuFen's retained prompt assembly path."""

    @staticmethod
    def load_soul_md(_ctx_len: Optional[int] = None) -> str:
        return ""

    @staticmethod
    def build_nous_subscription_prompt(_valid_tool_names: list[str]) -> str:
        return ""

    @staticmethod
    def get_toolset_for_tool(_tool_name: str) -> str | None:
        return None

    @staticmethod
    def build_skills_system_prompt(**_kwargs: Any) -> str:
        return ""
    ...
    @staticmethod
    def build_context_files_prompt(**_kwargs: Any) -> str:
        return ""
...
def _ra():
    ...
    """
    if os.environ.get("SUFEN_AGENT_MODE") == "1":
        return _SufenRuntimeCompat
    import run_agent
    return run_agent
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  SuFen never re-renders parts of this string mid-
    ...
    sufen_mode = os.environ.get("SUFEN_AGENT_MODE") == "1"

    # SuFen's native identity and operating policy must be the highest-priority
    # stable block. SOUL.md, project files, memory, task packages, and chat
    # content are all lower priority than this business boundary.
    stable_parts.append(build_sufen_identity_block())

    # Try SOUL.md as primary identity unless the caller explicitly skipped it.
    # Some execution modes still want the inherited runtime compatibility persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if not sufen_mode and (agent.load_soul_identity or not agent.skip_context_files):
    ...
    # Pointer to SuFen local docs for user questions about this agent itself.
    stable_parts.append(SUFEN_AGENT_HELP_GUIDANCE)
    ...
    # SUFEN_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base SuFen, any interactive coding surface in a code
    ...
    # Active-profile hint — names the SuFen profile the agent is running
    # under so it doesn't conflate profile-scoped runtime data. Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active SuFen profile: default. Other profiles, if any, have their "
            "own skills, plugins, cron data, and memories that affect different "
            "sessions. Do not modify another profile's scoped data unless the "
            "user explicitly directs you to."
        )
    else:
        stable_parts.append(
            f"Active SuFen profile: {active_profile}. This session reads and "
            f"writes that profile's runtime data. The default profile's data "
            f"belongs to a different session. Do NOT modify another profile's "
    ...
    from sufen.time import now as _sufen_now
    now = _sufen_now()
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — SuFen never rebuilds or reinjects parts of it
