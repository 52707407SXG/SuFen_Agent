...
def _diff_ansi() -> dict[str, str]:
    ...
    try:
        if os.getenv("SUFEN_AGENT_MODE") == "1":
            raise RuntimeError("SuFen mode uses built-in diff colors")
        from sufen_cli.skin_engine import get_active_skin
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in SuFen's inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("SUFEN_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
