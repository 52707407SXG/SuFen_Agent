...
from sufen_constants import PARTIAL_STREAM_STUB_ID
from sufen_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for SuFen tool use: "
    ...
    return (
        ...
        f"context, but SuFen needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In SuFen config, set `model.ollama_num_ctx: 65536` "
...
def _nous_entitlement_message(capability: str) -> str:
    try:
        from sufen_cli.nous_account import (
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    try:
        from sufen_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_start",
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `sufen fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from sufen_cli.moa_config import decode_moa_turn
    ...
    # all run inside Codex). Default SuFen path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for SuFen internals.
        #
        # SuFen invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for SuFen tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── inherited portal rate limit guard ──────────────────────
            ...
            if agent.provider == "nous":
                try:
                    from agent.managed_rate_guard import (
                        managed_rate_limit_remaining,
                        format_remaining as _fmt_managed_remaining,
                    )
                    _managed_remaining = managed_rate_limit_remaining()
                    if _managed_remaining is not None and _managed_remaining > 0:
                        _managed_msg = (
                            f"inherited portal rate limit active — "
                            f"resets in {_fmt_managed_remaining(_managed_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_managed_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_managed_msg}")
            ...
            try:
                ...
                try:
                    from sufen_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("SUFEN_DUMP_REQUESTS"):
                ...
                from sufen_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a SuFen/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if agent.provider == "nous":
                    try:
                        from agent.managed_rate_guard import clear_managed_rate_limit
                        clear_managed_rate_limit()
                    except Exception:
                        pass
                ...
                if (
                    ...
                    from sufen_constants import display_sufen_home as _dhh_fn
                    _dhh = _dhh_fn()
                    _body_text = ""
                    try:
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: sufen auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `sufen doctor` for credential-chain diagnostics, or")
                    ...
                    from sufen_constants import display_sufen_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for SuFen-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: sufen doctor set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: sufen doctor set ANTHROPIC_API_KEY \"\"")
                ...
                # ── inherited portal: record rate limit & skip retries ─────
                ...
                #
                # IMPORTANT: inherited portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, SuFen).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Nous model for minutes.  We use
                # ``is_genuine_managed_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_managed_rate_limit = False
                    try:
                        from agent.managed_rate_guard import (
                            is_genuine_managed_rate_limit,
                            record_managed_rate_limit,
                        )
                        ...
                        _genuine_managed_rate_limit = is_genuine_managed_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_managed_rate_limit:
                            record_managed_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                ...
                # which SuFen's system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. SuFen's system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`sufen",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `sufen auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `sufen doctor`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 inherited portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: sufen portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.nousresearch.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; inherited portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         inherited portal won't recognize that model name. Either switch to a", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: sufen doctor", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        sufen fallback add   (interactive picker — same as `sufen doctor`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.sufen/config.yaml` to extend the per-call "
                            f"timeout. (SuFen's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates SuFen mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from sufen_cli.plugins import get_pre_verify_continue_message, has_hook
