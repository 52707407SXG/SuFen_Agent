...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from toolsets import _SUFEN_CORE_TOOLS

        _core_tool_names = set(_SUFEN_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, SuFen expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``sufen_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_sufen_home()`` themselves.
        """
        if "sufen_home" not in kwargs:
            from sufen_constants import get_sufen_home
            kwargs["sufen_home"] = str(get_sufen_home())
        for provider in self._providers:
            try:
