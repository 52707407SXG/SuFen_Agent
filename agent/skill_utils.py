...
from sufen_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``SUFEN_KANBAN_TASK`` / ``SUFEN_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("SUFEN_KANBAN_TASK") or os.getenv("SUFEN_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        try:
            from sufen_constants import is_container

            result = is_container()
        except Exception:
            result = False
    elif env == "s6":
        # The SuFen Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``sufen_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SUFEN_PLATFORM`` or
            ``SUFEN_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``sufen`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.sufen/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``sufen`` cold-start time
    ...
    from sufen_constants import get_sufen_home

    sufen_home = get_sufen_home()
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SUFEN_HOME, not cwd
        if not p.is_absolute():
            p = (sufen_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.sufen/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: SuFen can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    sufen = metadata.get("sufen") or {}
    if not isinstance(sufen, dict):
        sufen = {}
    return {
        "fallback_for_toolsets": sufen.get("fallback_for_toolsets", []),
        "requires_toolsets": sufen.get("requires_toolsets", []),
        "fallback_for_tools": sufen.get("fallback_for_tools", []),
        "requires_tools": sufen.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    sufen = metadata.get("sufen")
    if not isinstance(sufen, dict):
        return []
    raw = sufen.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes SuFen metadata, VCS, virtualenv/dependency, cache, and skill
