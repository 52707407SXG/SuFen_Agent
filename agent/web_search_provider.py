...
``kind: backend``) or ``~/.sufen/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sufen doctor``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``sufen doctor`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``sufen doctor`` picker.

        Used by ``sufen_cli/tools_config.py`` to inject this provider as a
