...
Env vars::
    ...
    SUFEN_TAVILY_API_KEY=...     # https://app.tavily.com/home (required)
    SUFEN_TAVILY_BASE_URL=...    # optional override of https://api.tavily.com
"""
...
def _sufen_agent_mode() -> bool:
    return os.getenv("SUFEN_AGENT_MODE") == "1" or bool(os.getenv("SUFEN_TAVILY_API_KEY"))


def _tavily_api_key() -> str:
    if _sufen_agent_mode():
        value = os.getenv("SUFEN_TAVILY_API_KEY", "").strip()
        if value:
            return value
        try:
...
def _tavily_base_url() -> str:
    if _sufen_agent_mode():
        return os.getenv("SUFEN_TAVILY_BASE_URL", "https://api.tavily.com").strip() or "https://api.tavily.com"
    return os.getenv("TAVILY_BASE_URL", "https://api.tavily.com").strip() or "https://api.tavily.com"
...
def _tavily_request(endpoint: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    when the Tavily key is unset; the caller catches and surfaces as
    ...
    if not api_key:
        raise ValueError(
            "SUFEN_TAVILY_API_KEY environment variable not set. "
...
class TavilyWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        """Return True when SuFen's Tavily key is set to a non-empty value."""
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [
                {
                    "key": "SUFEN_TAVILY_API_KEY",
                    "prompt": "SuFen Tavily API key",
