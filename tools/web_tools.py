#!/usr/bin/env python3
"""SuFen web_search and web_extract tools.

The first-release SuFen runtime exposes Tavily only. It reads
``SUFEN_TAVILY_API_KEY`` from process env or local ``.env`` via
``sufen.config`` and deliberately ignores generic or other-agent keys while
``SUFEN_AGENT_MODE=1``.
"""
...
def _sufen_agent_mode() -> bool:
    """Return True when this inherited module is running under SuFen."""

    return os.getenv("SUFEN_AGENT_MODE", "1") != "0"


if TYPE_CHECKING and not _sufen_agent_mode():
...
if _sufen_agent_mode():
    ...
    def _unsupported_sufen_web_provider(*_args, **_kwargs):
        raise ValueError("SuFen first release enables Tavily only; set SUFEN_TAVILY_API_KEY.")

    def _firecrawl_backend_help_suffix() -> str:
        return ""

    def _get_firecrawl_client():
        return _unsupported_sufen_web_provider()
    ...
    def _peek_nous_access_token() -> str:
        return ""

    def _read_nous_access_token() -> str:
        return ""
    ...
    def nous_tool_gateway_unavailable_message(*_args, **_kwargs) -> str:
        return ""

    def prefers_gateway(*_args, **_kwargs) -> bool:
        return False
else:
    from plugins.web.firecrawl.provider import (
        Firecrawl,  # noqa: F401  # re-exported for inherited tests.
    ...
    from tools.managed_tool_gateway import (  # noqa: F401
...
SUFEN_WEB_ENV_MAP = {
    "TAVILY_API_KEY": "SUFEN_TAVILY_API_KEY",
}


def _sufen_env_name(name: str) -> str | None:
    if not _sufen_agent_mode():
        return None
    return SUFEN_WEB_ENV_MAP.get(name)


def _env_value(name: str) -> str:
    """Resolve ``name`` via SuFen config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through SuFen's config/.env layer (``sufen doctor set``, ``sufen doctor``)
    ...
    """
    mapped_name = _sufen_env_name(name)
    if mapped_name:
        value = os.getenv(mapped_name, "").strip()
        if value:
            return value
        try:
            from sufen.config import load_settings

            if mapped_name == "SUFEN_TAVILY_API_KEY":
    ...
    try:
        from sufen_cli.config import get_env_value
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.sufen/config.yaml."""
    if _sufen_agent_mode():
        return {}
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``sufen doctor``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if _sufen_agent_mode() and backend != "tavily":
        return False
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `sufen doctor` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    refresh against an inherited account service on every CLI startup (invoked at
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        from sufen_constants import get_sufen_dir

        cache_dir = get_sufen_dir("cache/web", "web_cache")
...
def _ensure_web_plugins_loaded() -> None:
    ...
    if _sufen_agent_mode():
        try:
            from agent.web_search_registry import get_provider, register_provider
            from plugins.web.tavily.provider import TavilyWebSearchProvider

            if get_provider("tavily") is None:
                register_provider(TavilyWebSearchProvider())
        except Exception as exc:  # noqa: BLE001
            logger.warning("SuFen Tavily provider registration failed: %s", exc)
        return

    try:
        from sufen_cli.plugins import _ensure_plugins_discovered
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `sufen doctor` to set one up."
                ),
            }
        else:
            logger.info(
