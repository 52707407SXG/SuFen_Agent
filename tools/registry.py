"""Central registry for all sufen-agent tools.
...
SUFEN_FIRST_RELEASE_TOOL_MODULES = {
    "sufen_mystand_tools",
    "web_tools",
}
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    sufen_mode = os.getenv("SUFEN_AGENT_MODE") == "1" and tools_dir is None
    module_names = [
        f"tools.{path.stem}"
        for path in sorted(tools_path.glob("*.py"))
        if path.name not in {"__init__.py", "registry.py", "mcp_tool.py"}
        and (not sufen_mode or path.stem in SUFEN_FIRST_RELEASE_TOOL_MODULES)
...
# timescales. Cache results for ~30 s so env-var flips via ``sufen doctor``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``sufen doctor enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("sufen_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``sufen_plugins.{name}``), not the exact module string.
                # A handler defined in ``sufen_plugins.pkg.handlers`` is
                # still owned by the ``sufen_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("sufen_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``sufen doctor enable foo``)
