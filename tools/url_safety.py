...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for SuFen-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``SUFEN_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("SUFEN_ALLOW_PRIVATE_URLS", "").strip().lower()
    ...
    # 2. Config file
    try:
        from sufen_cli.config import read_raw_config
        cfg = read_raw_config()
...
def is_safe_url(url: str) -> bool:
    ...
    ``SUFEN_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
