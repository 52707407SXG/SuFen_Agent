...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_sufen_home() under the active profile.
...
#   - cli.py, sufen_cli/*      -> inline on startup (no event loop)
#   - tui_gateway/server.py     -> inline on startup (no event loop)
#   - acp_adapter/server.py     -> asyncio.to_thread on session init

# Plugin tool discovery (user/project/pip plugins). SuFen first release keeps
# plugins closed by default so imports cannot read inherited local state.
if os.environ.get("SUFEN_AGENT_MODE") != "1":
    try:
        from sufen_cli.plugins import discover_plugins
        discover_plugins()
...
def get_tool_definitions(
    ...
    if quiet_mode:
        if os.environ.get("SUFEN_AGENT_MODE") == "1":
            ...
            try:
                from sufen_cli.config import get_config_path
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("SUFEN_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("SUFEN_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by SUFEN_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like sufen-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("sufen-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (sufen-*) include _SUFEN_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("sufen-")
    ...
    try:
        ...
        if os.environ.get("SUFEN_AGENT_MODE") == "1":
            logger.debug("Schema sanitization skipped in SuFen mode: %s", e)
        else:
    ...
    # window). Core SuFen tools (toolsets._SUFEN_CORE_TOOLS) are NEVER
    ...
    try:
        ...
        if os.environ.get("SUFEN_AGENT_MODE") == "1":
            logger.debug("Tool search assembly skipped in SuFen mode: %s", e)
        else:
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from sufen_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to sufen-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from sufen_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from sufen_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from sufen_cli.plugins import get_pre_tool_call_block_message
        ...
        try:
            ...
            from sufen_cli.middleware import run_tool_execution_middleware
