"""Centralized logging setup for SuFen-Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.sufen/logs/`` (profile-aware via ``get_sufen_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``sufen_cli.web_server``,
    ``sufen_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in SuFen (TUI, gateway, ``hy_memory`` server, MCP
...
from sufen_constants import get_config_path, get_sufen_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_sufen_session_injector", False):
    ...
    _session_record_factory._sufen_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``sufen doctor --component``.
COMPONENT_PREFIXES = {
    ...
    # ``sufen doctor --component gateway``.
    "gateway": ("gateway", "sufen_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("sufen_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "sufen_cli.web_server",
        "sufen_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the SuFen logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    sufen_home
        Override for the SuFen home directory.  Falls back to
        ``get_sufen_home()`` (profile-aware).
    log_level
    ...
    home = sufen_home or get_sufen_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_sufen_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._sufen_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the sufen group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from sufen_cli.config import is_managed
...
# the module header): when several SuFen processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._sufen_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="sufen-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_sufen_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        if config_path.exists():
            ...
            try:
                from sufen_cli import managed_scope
