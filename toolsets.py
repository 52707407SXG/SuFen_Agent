...
_SUFEN_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on SUFEN_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (SUFEN_KANBAN_TASK env set) or the current
...
SUFEN_FIRST_RELEASE_TOOLSETS = {"sufen"}
...
def _sufen_toolset_allowed(name: str) -> bool:
    return not _sufen_agent_mode() or name in SUFEN_FIRST_RELEASE_TOOLSETS
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `sufen tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SUFEN_KANBAN_TASK env "
    ...
    # Coding posture (base SuFen — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in the inherited tools config).
        "posture": True,
    },
    
    # ==========================================================================
    # Full SuFen toolsets (CLI + messaging platforms)
    #
    ...
    # the platform send CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "sufen-cli": {
        ...
        "tools": _SUFEN_CORE_TOOLS,
        "includes": []
    },

    "sufen-cron": {
        # Mirrors sufen-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then SuFen tools config filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as sufen-cli; gated by SuFen tools config",
        "tools": _SUFEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "sufen-email": {
        "description": "Email bot toolset - interact with SuFen via email (IMAP/SMTP)",
        "tools": _SUFEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "sufen-sms": {
        "description": "SMS bot toolset - interact with SuFen via SMS (Twilio)",
        "tools": _SUFEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "sufen-webhook": {
        ...
        "tools": _SUFEN_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "sufen-gateway": {
        ...
        "tools": [],
        "includes": ["sufen-telegram", "sufen-discord", "sufen-whatsapp", "sufen-slack", "sufen-signal", "sufen-bluebubbles", "sufen-homeassistant", "sufen-email", "sufen-sms", "sufen-mattermost", "sufen-matrix", "sufen-dingtalk", "sufen-feishu", "sufen-wecom", "sufen-wecom-callback", "sufen-weixin", "sufen-qqbot", "sufen-webhook", "sufen-yuanbao"]
    }
}
...
def get_toolset(name: str, *, include_registry: bool = True) -> Optional[Dict[str, Any]]:
    ...
    """
    if not _sufen_toolset_allowed(name):
        return None
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``sufen-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SUFEN_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``sufen-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if name in {"all", "*"}:
        if _sufen_agent_mode():
            return resolve_toolset("sufen", visited.copy(), include_registry=include_registry)
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (sufen-<name>).
        # Gives them _SUFEN_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("sufen-"):
            platform_name = name[len("sufen-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SUFEN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
...
def get_all_toolsets() -> Dict[str, Dict[str, Any]]:
    ...
    """
    if _sufen_agent_mode():
        sufen_toolset = get_toolset("sufen")
        return {"sufen": sufen_toolset} if sufen_toolset else {}
...
def get_toolset_names() -> List[str]:
    ...
    """
    if _sufen_agent_mode():
        return sorted(SUFEN_FIRST_RELEASE_TOOLSETS)
...
def validate_toolset(name: str) -> bool:
    ...
    if name in {"all", "*"}:
        return True
    if _sufen_agent_mode():
        return name in SUFEN_FIRST_RELEASE_TOOLSETS
    if name in TOOLSETS:
        return True
...
def create_custom_toolset(
    ...
    """
    if _sufen_agent_mode() and name not in SUFEN_FIRST_RELEASE_TOOLSETS:
        raise ValueError("SuFen first release only allows the 'sufen' toolset")
    TOOLSETS[name] = {
        "description": description,
        "tools": tools or [],
