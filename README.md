...
SuFen 是 My Stand 的轻量超级聊天军师：面向业主、客户、经纪人和售后档案，基于授权资料、入口指定知识图谱、经纪人特征卡、只读人工 memory 和 SuFen 日志摘要，输出策略建议、事件草稿和字段 diff 草稿。SuFen 不写长期 memory，`memoryPatch` 必须保持为空。
...
本机对话入口沿用站小伴同款终端入口结构：顶部 `Welcome back!`
启动卡、`SuFen` 标识、当前模型状态栏和 `❯` 提示符；HTTP/API
响应合同仍是 JSON。
...
时会 fail closed；带入授权档案上下文、经纪人特征卡、入口指定知识图谱和
dialogueLogKey 后，第一版业主房源档案场景会返回策略建议、事件草稿和
字段 diff 草稿。
...
- 长期 memory 只有一个根目录，由刚哥和元老师维护；SuFen 只能只读检索，不能创建 scoped memory 路径或 patch draft。
- 默认工具集是 SuFen 白名单：授权解析、授权档案读取适配位、知识图谱读取适配位、解析、web search/extract、只读人工 memory search、事件草稿、字段 diff 草稿。
